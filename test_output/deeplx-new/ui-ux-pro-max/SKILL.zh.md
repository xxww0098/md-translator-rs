...
- `loading-buttons`- 使用点击/轻点进行主要互动；不要仅仅依赖悬停
...
- `scroll-behavior`- 固定导航栏/底部栏必须为底层内容预留安全衬垫
...
- `whitespace-balance`- 数据列、价格和计时器使用表格/单倍行距数字，以防止布局偏移
...
- 在 375px（小型手机）和横向方向上进行测试********
- 验证启用减小运动量和最大尺寸动态类型时的行为
